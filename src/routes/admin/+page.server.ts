...
export const load: PageServerLoad = async ({ cookies, url }) => {
	...
	const [roles, rolePlans, items] = selectedGuildId
		? await Promise.all([
				getGuildRoles(selectedGuildId),
				listRolePlans(selectedGuildId, true),
				listItemDefinitions(selectedGuildId, true)
			])
		: [[], [], []];
	return {
		user,
...
export const actions: Actions = {
	...
	grantItem: async ({ cookies, request }) => {
		...
		const rawQuantity = String(form.get('quantity') || '').trim();
		if (!/^\d{17,20}$/.test(targetId))
			return fail(400, { message: '검색 결과에서 지급 대상을 선택해 주세요.' });
		if (!/^\d+$/.test(itemId)) return fail(400, { message: '지급할 아이템을 선택해 주세요.' });
		if (!/^[1-9]\d{0,8}$/.test(rawQuantity))
			return fail(400, { message: '지급 수량은 1~999,999,999 사이의 정수여야 합니다.' });
		const db = await getDB();
		const permission = await db`
			SELECT permissions FROM user_guilds
			WHERE user_id=${user.id} AND guild_id=${guildId} LIMIT 1
		...
		try {
			const item = await getItemDefinition(guildId, itemId);
			if (!item.active) return fail(400, { message: '비활성 아이템은 지급할 수 없습니다.' });
			const quantity = Number(rawQuantity);
			const totalQuantity = await changeInventoryQuantity({
				guildId,
				userId: targetId,
			...
			return {
				success: true,
				message: `${member.nick || member.user.username}님에게 ${item.iconEmoji} ${item.name} ${quantity}개를 지급했습니다. 현재 보유 수량: ${totalQuantity}개`
			};
		} catch (error) {
			if (error instanceof ItemNotFoundError)
				return fail(400, { message: '이 서버에 존재하는 아이템을 선택해 주세요.' });
			if (error instanceof ItemStackLimitError)
				return fail(400, { message: '지급 후 수량이 아이템의 최대 보유 수량을 초과합니다.' });
			throw error;
		}
	},
	createItem: async ({ cookies, request }) => {
		const user = await getSessionUser(cookies);
		if (!user) return fail(401, { message: '로그인이 필요합니다.' });
		const form = await request.formData();
		const guildId = String(form.get('guildId') || '');
		...
		if (permission.length !== 1 || !canManageGuild(String(permission[0].permissions)))
			return fail(403, { message: '서버 관리 권한이 필요합니다.' });
		try {
			await createItemDefinition(guildId, {
				key: `item_${crypto.randomUUID().replaceAll('-', '')}`,
				name,
				description,
				iconEmoji,
				type: itemKind === 'currency' ? 'consumable' : 'collectible',
	...
	rolePlan: async ({ cookies, request }) => {
		...
		const db = await getDB();
		const permission =
			await db`SELECT permissions FROM user_guilds WHERE user_id=${user.id} AND guild_id=${guildId}`;
		if (!permission.length || !canManageGuild(String(permission[0].permissions)))
			return fail(403, { message: '서버 관리 권한이 필요합니다.' });
		if (!amount) return fail(400, { message: '월 요금은 0.01 이상이어야 합니다.' });
		const role = (await getGuildRoles(guildId)).find(
			(item) => item.id === roleId && !item.managed && item.name !== '@everyone'
		);
	...
	disableRolePlan: async ({ cookies, request }) => {
		...
		await Promise.all(
			subscriptions.map((row: Record<string, unknown>) =>
				removeGuildMemberRole(guildId, String(row.user_id), String(row.role_id)).catch(
					console.error
				)
			)
		);
