...
export const GET: RequestHandler = async ({ cookies, params, url }) => {
	...
	const membership = await db`
		...
		WHERE user_id = ${user.id} AND guild_id = ${params.guildId} LIMIT 1
	`;
	if (membership.length !== 1) error(403, 'Guild access denied.');
	const canIncludeSelf = includeSelf && canManageGuild(String(membership[0].permissions));

	const members = await searchGuildMembers(params.guildId, query);
	return json({
		members: members
			.filter((member) => !member.user.bot && (canIncludeSelf || member.user.id !== user.id))
			.map((member) => ({
				id: member.user.id,
