<script lang="ts">
	...
	function createProp(event: SubmitEvent) {
		...
		propRequestId = send({
			...
			teleportX: propAction === 'teleport' ? form.get('teleportX') : null,
			teleportY: propAction === 'teleport' ? form.get('teleportY') : null
		});
	}
	...
	function beginRoom(event: PointerEvent) {
		...
		(event.currentTarget as HTMLElement).setPointerCapture(event.pointerId);
		start = buildTool === 'wall' || buildTool === 'door' ? wallPoint(event) : cell(event);
		end = start;
	}
	...
	function resizeRoom(event: PointerEvent) {
		...
		if (!building || !start || !(event.currentTarget as HTMLElement).hasPointerCapture(event.pointerId))
			return;
		end = buildTool === 'wall' || buildTool === 'door' ? wallPoint(event) : cell(event);
	}
