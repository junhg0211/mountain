...
<main>
	...
	<section><h2>색상 역할 구독</h2><p class="warning">가입 즉시 한 달 요금 전액이 결제되고, 이후 매월 1일 12:00(KST)에 결제됩니다. 일할 계산과 환불은 없습니다.</p>
		<div class="cards">{#each data.plans as plan}<article><h3>{plan.name}</h3><p>매월 {plan.monthlyPrice}</p><form method="POST" action="?/subscribe" use:enhance><input type="hidden" name="guildId" value={data.selectedGuildId}><input type="hidden" name="planId" value={plan.id}><button>구독하기</button></form></article>{/each}</div>
		{#each data.subscriptions as item}<div class="row"><span><b>{item.name}</b> · {item.status === 'active' ? `다음 결제 ${date(item.nextChargeAt)}` : '해지됨'}</span>{#if item.status === 'active'}<form method="POST" action="?/cancelSubscription" use:enhance><input type="hidden" name="guildId" value={data.selectedGuildId}><input type="hidden" name="id" value={item.id}><button class="secondary">해지</button></form>{/if}</div>{/each}
	</section>
